        ...
        <Table>
          ...
          <TableBody>
            {payments.length > 0 ? payments.map(p => (
              ...
              <TableRow>
                <TableCell colSpan={4} className="h-24 text-center text-[12px]">Aucun paiement enregistré pour ce contrat.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
...
export default function PaymentTable({ rentals, payments, onAddPaymentForRental }: { 
  ...
  const columns: ColumnDef<Rental>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
          const rental = row.getIsGrouped() ? row.subRows[0]?.original : row.original;
          if (!rental) return <Badge variant="outline" className="text-[11px] w-[80px] flex justify-center">N/A</Badge>;
